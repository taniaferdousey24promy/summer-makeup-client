...
const MyCart = () => {
  ...
  const total = cart.reduce(
    (sum, singleClasses) => parseFloat(singleClasses.price) + sum,
    0
  );
  ...
  const handleDelete = (singleClasses) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(
          `https://summer-makeup-server-taniaferdousey24promy.vercel.app/carts/${singleClasses._id}`,
          {
            method: "DELETE",
          }
        )
          ...
          <tbody>
            {cart.map((singleClasses, index) => (
              <tr key={singleClasses._id}>
                ...
                <td>
                  <button
                    onClick={() => handleDelete(singleClasses)}
                    className="btn btn-red-600 btn-ghost btn-lg"
                  >
                    <FaTrashAlt></FaTrashAlt>
                  </button>
                </td>
                <td>
                  <button className="btn btn-red-600 btn-ghost btn-lg">
